...
void fht(ll* p, int n, bool inv){
  ...
  fore(j, 0, l) { ll u = p[i + j], v = p[i + l + j];
...
vector<ll> multiply(vector<ll>& p1, vector<ll>& p2){
  ...
  fore(i, 0, n) c1[i] = 0, c2[i] = 0; fore(i,0,sz(p1))c1[i]=p1[i];
  fore(i, 0, sz(p2)) c2[i] = p2[i];fht(c1, n, false);fht(c2,n,false);
  fore(i, 0, n) c1[i] *= c2[i]; fht(c1, n,true);return vi(c1,c1+n);}
void fht(vector<ll>& p, bool inv) {
fore(i,0,sz(p))c1[i]=p[i];fht(c1,sz(p),inv);fore(i,0,sz(p))p[i]=c1[i];}
