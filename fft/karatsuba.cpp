...
ll* ini(int n){ll *r=new ll[n];fill(r,r+n,0);return r;}
void karatsura(int n, ll* p, ll* q, ll* r){
  if(n<=0)return;
  ...
  if(n<35)fore(i,0,n)fore(j,0,n)r[i+j]+=p[i]*q[j] % MOD, r[i + j] %= MOD;
    ...
    ll *a=p,*b=p+nac,*c=q,*d=q+nac;
    ll *ab=ini(nbd+1),*cd=ini(nbd+1),*ac=ini(nac*2),*bd=ini(nbd*2);
...
vector<ll> multiply(vector<ll> p0, vector<ll> p1){
  int n=max(p0.size(),p1.size()); ll *p=ini(n),*q=ini(n),*r=ini(2*n);
  fore(i,0,p0.size())p[i]=p0[i]; fore(i,0,p1.size())q[i]=p1[i];
  karatsura(n,p,q,r); vector<ll> rr(r,r+p0.size()+p1.size()-1);
  free(p);free(q);free(r); return rr; }