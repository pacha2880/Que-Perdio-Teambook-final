...
const int MAXN = 1<<20;typedef ll tf;
...
typedef vector<tf> poly; const tf MOD=998244353, RT=3;
...
CD cp1[MAXN+9],cp2[MAXN+9]; int R[MAXN+9];
void dft(CD* a, int n, bool inv){
  ...
  for(int m=2;m<=n;m*=2){
    ...
    for(int j=0;j<n;j+=m){ CD w(1);
      for(int k=j,k2=j+m/2;k2<j+m;k++,k2++){
        CD u=a[k];CD v=a[k2]*w;a[k]=u+v;a[k2]=u-v;w=w*wi; } } }
  // if(inv)fore(i,0,n)a[i]/=n; // FFT
  if(inv){ // NTT
    CD z(pot(n,MOD-2)); fore(i,0,n)a[i]=a[i]*z; } }
poly multiply(poly& p1, poly& p2){
  int n=p1.size()+p2.size()+1, m=1,cnt=0;
  while(m<=n)m+=m,cnt++;
  ...
  fore(i,0,m)cp1[i]=cp1[i]*cp2[i]; dft(cp1,m,true);
  poly res; n-=2;
