...
Usage: linearRec({0,1},{1,1},k) k’th Fibonacci number O(n^2logk)*/
typedef vector<ll> Poly;
  ll linearRec(Poly S, Poly tr, ll k) { int n = sz(tr);
  auto combine = [&](Poly a, Poly b) { Poly res(n * 2 + 1);
  ...
  for (++k; k; k /= 2) { if (k % 2) pol = combine(pol, e);
    e = combine(e, e); }
  ll res = 0; fore(i,0,n) res = (res + pol[i + 1] * S[i]) % mod;
  return res; }
