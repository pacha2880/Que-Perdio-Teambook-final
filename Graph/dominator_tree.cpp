...
void dfspre(int pos){
  ...
  for(auto x:g[pos]){
    rev[x].pb(pos); if(rnk[x]==n) pre[x]=pos,dfspre(x); } }
int eval(int v){
...
void dominators(int rt){
  fore(i,0,n){
    ...
    rnk[i]=pre[i]=anc[i]=idom[i]=n; semi[i]=low[i]=i; }
  ord.clear(); dfspre(rt);
  for(int i=sz(ord)-1;i;i--){
    int w=ord[i];
    for(int v:rev[w]){
      int u=eval(v); if(rnk[semi[w]]>rnk[semi[u]])semi[w]=semi[u]; }
  ...
  fore(i,0,n) if(idom[i]==n)idom[i]=-1; else dom[idom[i]].pb(i); }