//https://vjudge.net/problem/SPOJ-DYNALCA
#include <bits/stdc++.h>
#include <cassert>
#include <cstddef>
#include <streambuf>
...
#define initsg int new_node= node*2,mid = (iz+der)/2; 
#define clr(a,b) memset(a,b,sizeof(a));
const long long tam=333;
const int K = 300;
...
void solve() {
    ...
    forr(i, 0, q) {
        cin>>type>>iz>>der;
        if (type == 2) {
            auto res = tree.qry(1, n, 1, iz, der);
            cout<<res.sumpw2<<"\n";
        } else if (type == 1) {
            cin>>dx;
            mylazy tolazy = {dx,INF};
            tree.upd1(1, n, 1, iz,der, tolazy);
        } else if (type == 0) {
            cin>>dx;
            mylazy tolazy = {0, dx};
            tree.upd1(1, n, 1, iz,der, tolazy);
