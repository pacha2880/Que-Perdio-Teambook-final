namespace CHT {
    bool Q = 0;const ll inf = 1e18;
    struct Line {
        mutable ll m, b, x;
        // Maximo: m < ot.m     // Minimo: m > ot.m
    ...
    ll intersection (const Line &p, const Line &q) {return ceil(q.b - p.b, p.m - q.m);}
    struct Hull : multiset<Line> {
        bool valid (multiset<Line>::iterator  it) {
            ...
            auto ant = it, sig = it;    ant--, sig++;
            if (sig == end()) {
                it->x = intersection(*it, *ant);
                return 1;
            }
            ll x = intersection(*it, *ant), y = intersection(*it, *sig);
            if (x > y) return 0;
            it->x = x, sig->x = y;
            return 1;
        }
